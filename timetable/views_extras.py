...
def safe_redirect(request, fallback='/'):
    """Safely redirect back using HTTP_REFERER, validated against allowed hosts."""
...
def require_college_manager_or_system(view_func):
    def wrapper(request, *args, **kwargs):
        ...
        if request.user.user_type not in ('college_manager', 'department_head', 'system_manager'):
...
def notifications_delete(request):
    ...
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        ...
        return JsonResponse({'success': True, 'unread_count': unread, 'total': total})
    return safe_redirect(request, fallback='/')
...
def public_schedule_export(request):
    ...
    if not period_id:
        ...
        _messages.warning(request, 'الرجاء اختيار الفترة الأكاديمية أولاً قبل التصدير.')
        return _redirect('public_schedule')
    ...
    if fmt == 'csv':
        ...
        for lec in lectures:
            writer.writerow([
                'محاضرة',
                safe_csv_value(lec.course.course_name),
                safe_csv_value(lec.course.course_code),
                safe_csv_value(lec.professor.name),
                safe_csv_value(lec.room.name),
        ...
        for lab in labs:
            writer.writerow([
                'معمل',
                safe_csv_value(lab.course.course_name),
                safe_csv_value(lab.course.course_code),
                safe_csv_value(lab.professor.name),
                safe_csv_value(lab.hall.name),
...
def export_lectures_csv(request):
    ...
    for lec in lectures:
        writer.writerow([
            safe_csv_value(lec.department.name),
            safe_csv_value(lec.period.year.year_name),
            lec.period.semester_type,
            safe_csv_value(lec.course.course_name),
            safe_csv_value(lec.course.course_code),
            safe_csv_value(lec.professor.name),
            safe_csv_value(lec.room.name),
            safe_csv_value(lec.room.code),
            DAY_NAMES.get(lec.day_of_week, lec.day_of_week),
            str(lec.start_time),
            str(lec.end_time),
...
def export_labs_csv(request):
    ...
    for lab in labs:
        writer.writerow([
            safe_csv_value(lab.department.name),
            safe_csv_value(lab.period.year.year_name),
            lab.period.semester_type,
            safe_csv_value(lab.course.course_name),
            safe_csv_value(lab.course.course_code),
            safe_csv_value(lab.professor.name),
            safe_csv_value(lab.assistant.name) if lab.assistant else '',
            safe_csv_value(lab.hall.name),
            safe_csv_value(lab.hall.code),
            DAY_NAMES.get(lab.day_of_week, lab.day_of_week),
            str(lab.start_time),
            str(lab.end_time),
            lab.group_number or '',
...
@login_required
def student_schedule_pdf(request):
    from reportlab.lib.pagesizes import A4, landscape
    from reportlab.lib import colors
    from reportlab.lib.units import cm
    from reportlab.platypus import SimpleDocTemplate, Table, TableStyle, Paragraph, Spacer
    ...
    import arabic_reshaper
    from bidi.algorithm import get_display
    from .models import Student, LectureSchedule, LabSchedule
    ...
    try:
        student = Student.objects.select_related('period__year', 'period__department').get(id=request.user.user_id)
    except Student.DoesNotExist:
        messages.error(request, 'لم يتم العثور على بيانات الطالب')
        return redirect('login')

    period = student.period
    lectures = LectureSchedule.objects.filter(period=period).select_related(
        'course', 'professor', 'room'
    ).order_by('day_of_week', 'start_time')
    labs = LabSchedule.objects.filter(period=period).select_related(
        'course', 'professor', 'hall'
    ).order_by('day_of_week', 'start_time')
    ...
    tms_color = colors.HexColor('#17616c')
    purple_color = colors.HexColor('#6b3fa0')
    alt_bg = colors.HexColor('#f5f7fa')
    orange_bg = colors.HexColor('#fff8e7')
    ...
    section_style = ParagraphStyle('Section', parent=styles['Normal'], fontSize=12,
                                   fontName='Amiri-Bold', textColor=colors.white,
                                   alignment=TA_CENTER, spaceAfter=4)
    ...
    dept_name = period.department.name if period and period.department else ''
    year_name = period.year.year_name if period and period.year else ''
    sem = period.semester_type if period else ''
    sem_label = 'الفصل الأول' if sem == '1' else 'الفصل الثاني'

    elements.append(Paragraph(ar('الجدول الدراسي'), title_style))
    elements.append(Paragraph(ar(student.name), ParagraphStyle('Nm', parent=styles['Normal'],
                                fontName='Amiri-Bold', fontSize=13, textColor=purple_color,
                                alignment=TA_CENTER, spaceAfter=4)))
    elements.append(Paragraph(ar(f'{dept_name}  /  {year_name}  /  {sem_label}'), sub_style))
    elements.append(Paragraph(ar('جامعة السودان للعلوم والتكنولوجيا'), sub_style))
    elements.append(Paragraph(f'{datetime.now().strftime("%Y-%m-%d %H:%M")}', sub_style))
    elements.append(Spacer(1, 0.5*cm))

    lec_headers = [ar(h) for h in ['المادة', 'الأستاذ', 'القاعة', 'اليوم', 'البداية', 'النهاية']]
    lec_data = [lec_headers]
    for lec in lectures:
        day_ar = DAY_NAMES.get(lec.day_of_week, lec.day_of_week)
        lec_data.append([
            ar(lec.course.course_name[:30]),
            ar(lec.professor.name[:25]),
            ar(lec.room.name[:15]),
            ar(day_ar),
            str(lec.start_time)[:5],
            str(lec.end_time)[:5],
    ...
    if len(lec_data) > 1:
        lec_title_data = [[Paragraph(ar('المحاضرات'), section_style)]]
        lec_title = Table(lec_title_data, colWidths=[26*cm])
        lec_title.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, -1), tms_color),
            ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
            ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
            ('ROWHEIGHT', (0, 0), (-1, -1), 24),
            ('TOPPADDING', (0, 0), (-1, -1), 4),
        ]))
        elements.append(lec_title)
        col_widths = [7*cm, 6*cm, 4*cm, 3.5*cm, 2.75*cm, 2.75*cm]
        tbl = Table(lec_data, colWidths=col_widths, repeatRows=1)
        tbl.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), tms_color),
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.white),
            ('FONTNAME', (0, 0), (-1, 0), 'Amiri-Bold'),
    ...
    lab_headers = [ar(h) for h in ['المادة', 'الأستاذ', 'المعمل', 'اليوم', 'البداية', 'النهاية', 'المجموعة']]
    lab_data = [lab_headers]
    for lab in labs:
        day_ar = DAY_NAMES.get(lab.day_of_week, lab.day_of_week)
        lab_data.append([
            ar(lab.course.course_name[:30]),
            ar(lab.professor.name[:25]),
            ar(lab.hall.name[:15]),
            ar(day_ar),
            str(lab.start_time)[:5],
            str(lab.end_time)[:5],
            str(lab.group_number) if lab.group_number else '—',
        ])

    if len(lab_data) > 1:
        lab_title_data = [[Paragraph(ar('المعامل'), section_style)]]
        lab_title = Table(lab_title_data, colWidths=[26*cm])
        lab_title.setStyle(TableStyle([
        ...
        col_widths = [6*cm, 5.5*cm, 3.5*cm, 3.5*cm, 2.5*cm, 2.5*cm, 2.5*cm]
        tbl2 = Table(lab_data, colWidths=col_widths, repeatRows=1)
        tbl2.setStyle(TableStyle([
            ...
            ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
            ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
            ('ROWBACKGROUNDS', (0, 1), (-1, -1), [colors.white, orange_bg]),
            ('GRID', (0, 0), (-1, -1), 0.4, colors.HexColor('#CCCCCC')),
            ('ROWHEIGHT', (0, 0), (-1, -1), 20),
        ]))
        elements.append(tbl2)

    if len(lec_data) == 1 and len(lab_data) == 1:
        elements.append(Paragraph(ar('لا يوجد جدول دراسي مسجل لهذه الفترة'),
                                  ParagraphStyle('Empty', parent=styles['Normal'],
                                                 fontName='Amiri', fontSize=11,
                                                 alignment=TA_CENTER, textColor=colors.grey)))

    doc.build(elements)
    buffer.seek(0)
    safe_name = student.name.replace(' ', '_') if student.name else 'student'
    filename = f'schedule_{safe_name}_{datetime.now().strftime("%Y%m%d")}.pdf'
    response = HttpResponse(buffer.getvalue(), content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="{filename}"'
    return response
            ...
            try:
                if schedule_type == 'labs':
                    ...
                    lab = LabSchedule.objects.create(
                        ...
                        day_of_week=row['day_of_week'].strip().title(),
