...
# ─────────────────────────────────────────────────────────────────────────────
#  PROFILE VIEWING — hierarchy-based access
#  system_manager  → everyone (redirects to admin view)
#  college_manager → dept_head, professor, student (within their college)
#  department_head → professor (in college), student (in their department)
#  professor       → student (in their college)
# ─────────────────────────────────────────────────────────────────────────────

@login_required
def view_user_profile(request, username):
    viewer = request.user
    target = get_object_or_404(UnifiedUser, username=username)

    # System manager: redirect to full admin profile view
    if viewer.user_type == 'system_manager':
        return redirect('user_profile_admin', pk=target.pk)

    # Define what each role can see
    VIEWABLE = {
        'college_manager': ['department_head', 'professor', 'student'],
        'department_head':  ['professor', 'student'],
        'professor':        ['student'],
        'student':          [],
    }
    allowed_types = VIEWABLE.get(viewer.user_type, [])
    if target.user_type not in allowed_types:
        messages.error(request, 'ليس لديك صلاحية لعرض هذا الملف الشخصي')
        return redirect('home')

    # Scoped access checks
    if viewer.user_type in ('college_manager', 'department_head'):
        college = get_college_for_manager(viewer)

        if target.user_type == 'department_head':
            if target.college_id != college.id:
                messages.error(request, 'ليس لديك صلاحية لعرض هذا الملف الشخصي')
                return redirect('home')

        elif target.user_type == 'professor':
            in_college = (
                Professor.objects.filter(username=target.username, college=college).exists()
                or Professor.objects.filter(
                    username=target.username,
                    college_relations__college=college
                ).exists()
            )
            if not in_college:
                messages.error(request, 'ليس لديك صلاحية لعرض هذا الملف الشخصي')
        ...
        elif target.user_type == 'student':
            if viewer.user_type == 'department_head':
                if target.department_id != viewer.department_id:
                    messages.error(request, 'ليس لديك صلاحية لعرض هذا الملف الشخصي')
                    return redirect('home')
            else:
                if target.department_id and not CollegeDepartment.objects.filter(
                    college=college, department_id=target.department_id
                ).exists():
                    messages.error(request, 'ليس لديك صلاحية لعرض هذا الملف الشخصي')
                    return redirect('home')

    elif viewer.user_type == 'professor':
        prof_college = viewer.college
        if target.user_type == 'student' and target.department_id:
            if not CollegeDepartment.objects.filter(
                college=prof_college, department_id=target.department_id
            ).exists():
                messages.error(request, 'ليس لديك صلاحية لعرض هذا الملف الشخصي')
                return redirect('home')

    # Build profile data
    u_type         = target.user_type
    professor_obj  = None
    student_obj    = None
    lectures       = None
    labs           = None
    ...
    if u_type == 'professor':
        try:
            professor_obj  = Professor.objects.select_related('college').get(username=target.username)
            lectures       = LectureSchedule.objects.select_related('course', 'department', 'room').filter(professor=professor_obj).order_by('day_of_week', 'start_time')
            labs           = LabSchedule.objects.select_related('course', 'hall').filter(professor=professor_obj).order_by('day_of_week', 'start_time')
            lectures_count = lectures.count()
            labs_count     = labs.count()
        except Professor.DoesNotExist:
            pass
        ...
        try:
            student_obj = Student.objects.select_related('department', 'period__year').get(username=target.username)
        except Student.DoesNotExist:
            pass

    return render(request, 'timetable/user_profile_view.html', {
        'profile_user':   target,
...
def professor_student_list(request):
    """List students in the professor's college — professors can view their profiles."""
    prof_college = request.user.college
    dept_ids = list(
        CollegeDepartment.objects.filter(college=prof_college)
        .values_list('department_id', flat=True)
    )
    students = (
        Student.objects
        .select_related('department', 'period__year')
        .filter(department_id__in=dept_ids)
        .order_by('department__name', 'name')
    )
    return render(request, 'timetable/professor_students.html', {
        'students': students,
        'college':  prof_college,
    })
